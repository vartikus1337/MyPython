def requireAnswerTwoDoors(whatFirstAnswerTwoDoors: str, whatSecondAnswerTwoDoors: str, firstAnswerTwoDoors: str,
                          secondAnswerTwoDoors: str):
...
def requireAnswerThreeDoors(whatFirstAnswerThreeDoors: str, whatSecondAnswerThreeDoors: str,
                            whatThirdAnswerThreeDoors: str, firstAnswerThreeDoors: str, secondAnswerThreeDoors: str,
                            thirdAnswerThreeDoors):
    answerInRequireTreeDoors = 0
    while True:
        answerInRequireThreeDoors = int(input(
            f"1 - {whatFirstAnswerThreeDoors} 2 - {whatSecondAnswerThreeDoors} 3 - {whatThirdAnswerThreeDoors} \n"))
...
if answer == 1:
    print("Вы вошли в комнату \nДверь сзади захлопнулась \nВы почувствовали неприятный запах \nКомната была тёмная "
          "\nВы нащупали  3 двери ")
    answer = requireAnswerThreeDoors("дверь от которой воняло", "Закрытая дверь",
                                     "дверь, на которой вы нащупали число 107...",
                                     "При открытии этой двери вы зажали нос", "Вы силой открыли дверь",
                                     "При открытии двери вас затянуло туда нечто")
    if answer == 1:
        ...
        print(
            ...
            "долгое время подошла ваша очередь \nПеред вами был стол, на котором была свечка\nЗа столом сидел "
            "препод\nПрепод спросил:'на тройку с натяжкой?'\nВы согласились\nПрепод говорит:'на натяжку' и гасит "
            "свечку...") 
    else:
        print(
            "Вы оказались за партой\nВам дали одинарный листок, на котором было написано ФИО преподавателя\nЭто был "
            "счастливый билет\nВы ответили правильно и очутились дома...") 
else:
    print(
        "Вы зашли в хорошо освещённую комнату \nДверь захлопнулась \nВ ней были 2 дыры \nУ одной дыры была табличка, "
        "на которой написано: 'бесконечная дыра приветствия мира'\nОт второй дыры воняло ") 
    answer = requireAnswerTwoDoors("Дыра у которой была табличка", "Дыра, от которой воняло",
                                   "Вы прыгнули в дыру бесконечного приветствия мира",
                                   "Вы прыгнули в вонючую дыру, вы почувствовали неприятный запах")
    if answer == 1:
        ...
        print("Вы падали недолго\nВы приземлились на гору мусора\nВас встретили черепашки-ниндзя и увели куда-то\nНо "
              "куда и зачем история умалчивает...") 
